...
*/
  ...
  return (
    <div
      className="wrapper"
      onMouseDown={() => setMouseDown(true)} // Kullanıcı fareye tıkladığında, mouseDown durumunu true olarak ayarlar
      onMouseUp={() => setMouseDown(false)} // Kullanıcı fareyi bıraktığında, mouseDown durumunu false olarak ayarlar
      onMouseMove={(e) => {
        // Kullanıcı fareyi hareket ettirdiğinde çalışır
        if (mouseDown) {
          // Eğer kullanıcı fareye tıkladıysa
          const targetId = e.target.id // Hedef pikselin ID'sini alır
          setPixels(
            (
              prevPixels // Pikselleri günceller
            ) =>
              prevPixels.map((pixel) =>
                pixel.id === targetId // Eğer piksel hedeflenen piksele eşitse
                  ? { ...pixel, filled: wantsToDraw } // Pikselin durumunu wantsToDraw'a göre değiştirir
                  : pixel
              )
          )
        }
